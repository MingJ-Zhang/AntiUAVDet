...
# The default ``file_descriptor`` strategy transfers tensor storage handles via
# Unix ancillary data. Long single-GPU runs can exhaust the per-process file
# descriptor budget and fail with ``received 0 items of ancdata``. Named shared
# memory avoids that handle-transfer path while preserving multi-worker loading.
