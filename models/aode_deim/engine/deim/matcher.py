...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0,
                ...
                visibility_aware=False, visibility_cls_floor=0.35,
                visibility_geo_gain=1.0, visibility_geo_tau=0.35):
        """Creates the matcher
        ...
        self.visibility_cls_floor = float(visibility_cls_floor)
        self.visibility_geo_gain = float(visibility_geo_gain)
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False, epoch=0):
        ...
        if self.visibility_aware and outputs.get('target_visibility') is not None:
            vis_list = outputs['target_visibility']
            if len(vis_list) == len(targets):
                target_visibility = torch.cat([
                    vis.to(device=out_bbox.device, dtype=out_bbox.dtype) for vis in vis_list
                ]).clamp(0.02, 1.0)
        ...
        if self.change_matcher and epoch >= self.matcher_change_epoch:
            ...
            # Visibility-calibrated matching quality. For low-visibility tiny targets,
            # a smooth geometry term avoids an abrupt quality collapse from a few pixels.
            if target_visibility is not None:
                geo_dist = torch.cdist(out_bbox, tgt_bbox, p=1)
                geo_quality = torch.exp(-geo_dist / max(self.visibility_geo_tau, 1e-6))
                vis = target_visibility.unsqueeze(0)
                match_quality = vis * bbox_iou.clamp_min(0.0) + (1.0 - vis) * geo_quality
                cls_power = self.visibility_cls_floor + (1.0 - self.visibility_cls_floor) * vis
                C = (-1) * (class_score.clamp_min(1e-8).pow(cls_power) *
                            torch.pow(match_quality.clamp_min(1e-8), self.iou_order_alpha))
            else:
            ...
            # Visibility-calibrated costs: classification is trusted less when local
            # evidence is weak, while geometry receives a compensating weight.
            if target_visibility is not None:
                vis = target_visibility.unsqueeze(0)
                cls_weight = self.visibility_cls_floor + (1.0 - self.visibility_cls_floor) * vis
                geo_weight = 1.0 + self.visibility_geo_gain * (1.0 - vis)
                C = (self.cost_bbox * geo_weight * cost_bbox +
                     self.cost_class * cls_weight * cost_class +
                     self.cost_giou * geo_weight * cost_giou)
            else:
