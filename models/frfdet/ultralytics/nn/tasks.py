...
from ultralytics.nn.pool import (
    ConditionalPoolingLayer,
    PatchMerging,
)

from ultralytics.nn.necks import EVCBlock, AFPN
from ultralytics.nn.upsample import CARAFE, DySample, FADE
...
from .FRFDet import IBS_D, IBS_U, SFRCFMul, SFRCFAdd
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
        ...
        elif m in [SFRCFAdd, SFRCFMul]:
            c1, c2 = [ch[x] for x in f], sum(ch[x] for x in f)
            args = [c1, *args]
            n = 1
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
            if f == -1 and isinstance(ch[f], (list, tuple)):
                args.append(list(ch[f]))
            else:
            ...
            args = [c1, c2]
            n = 1
        elif m in [ConditionalPoolingLayer]:
            c2 = ch[f]
            args = [c2, *args]
            n = 1
        else:
