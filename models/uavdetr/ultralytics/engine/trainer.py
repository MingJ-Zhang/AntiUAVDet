...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        amp_dtype_name = os.getenv('ULTRALYTICS_AMP_DTYPE', 'float16').lower()
        if amp_dtype_name not in ('float16', 'bfloat16'):
            raise ValueError(f'Unsupported ULTRALYTICS_AMP_DTYPE={amp_dtype_name!r}')
        self.amp_dtype = torch.bfloat16 if amp_dtype_name == 'bfloat16' else torch.float16
        self.scaler = amp.GradScaler(enabled=self.amp and self.amp_dtype == torch.float16)
        if world_size > 1:
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                loss_is_finite = bool(torch.isfinite(self.loss.detach()).all())
                items_are_finite = bool(torch.isfinite(self.loss_items.detach()).all())
                if not (loss_is_finite and items_are_finite):
                    self.skipped_nonfinite_batches += 1
                    self.optimizer.zero_grad(set_to_none=True)
                    LOGGER.warning(
                        f'WARNING ⚠️ skipped non-finite batch epoch={epoch + 1} batch={i + 1} '
    ...
    def optimizer_step(self):
        ...
        if not bool(torch.isfinite(grad_norm)):
            ...
            self.scaler.update()
            LOGGER.warning(
                f'WARNING ⚠️ skipped non-finite gradient update '
                f'total_skipped={self.skipped_nonfinite_batches}')
            return False
    ...
    def resume_training(self, ckpt):
        ...
        self.skipped_nonfinite_batches = int(ckpt.get('skipped_nonfinite_batches', 0))
        if self.resume:
