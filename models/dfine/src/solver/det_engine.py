...
def train_one_epoch(
    ...
    **kwargs,
    ...
    amp_dtype_name = kwargs.get("amp_dtype", "float16")
    amp_dtype = {
        ...
        if scaler is not None:
            ...
            if not torch.isfinite(loss.detach()):
                ...
                print(
                    ...
                    "skipping the whole batch without optimizer/EMA/scheduler updates. "
                    f"losses={{{', '.join(f'{k}: {v.detach().item()}' for k, v in loss_dict.items())}}}"
                )
                continue
            ...
            if max_norm > 0:
                ...
                if not torch.isfinite(grad_norm):
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    print(
                        f"Non-finite gradient norm at epoch={epoch}, step={i}; "
                        "skipping the whole batch without optimizer/EMA/scheduler updates."
                    )
                    continue
    ...
    stats["skipped_nonfinite_batches"] = skipped_nonfinite_batches
    return stats
