...
class PatchSA(nn.Module):
    ...
    def _forward1(self, x):
        ...
        assert self.patch_size >= self.stride
        windows_h = (H + self.stride - 1) // self.stride
        windows_w = (W + self.stride - 1) // self.stride
        pad_h = (windows_h - 1) * self.stride + self.patch_size - H
        pad_w = (windows_w - 1) * self.stride + self.patch_size - W
        expan_x = F.pad(x, (0, pad_w, 0, pad_h), mode='replicate')
        repeat_x = [expan_x] * 3
        ...
        for i in range(0, H, self.stride):
            for j in range(0, W, self.stride):
                ...
                tile_h = min(self.stride, H - i)
                tile_w = min(self.stride, W - j)
                res[:, :, i: i + tile_h, j: j + tile_w] = _res[:, :, :tile_h, :tile_w]
    ...
    def _forward2(self, x):
        ...
        windows_h = (H + self.stride - 1) // self.stride
        windows_w = (W + self.stride - 1) // self.stride
        patch_num = windows_h * windows_w
        output_h = windows_h * self.stride
        output_w = windows_w * self.stride
        pad_h = (windows_h - 1) * self.stride + self.patch_size - H
        pad_w = (windows_w - 1) * self.stride + self.patch_size - W
        expan_x = F.pad(x, (0, pad_w, 0, pad_h), mode='replicate')
        repeat_x = [expan_x] * 3
        ...
        res = F.fold(_res, output_size=(output_h, output_w), kernel_size=self.stride, stride=self.stride)
        return self.to_out(res[:, :, :H, :W])
