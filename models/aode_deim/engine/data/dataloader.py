...
class BatchImageCollateFunction(BaseCollateFunction):
    def __init__(
        ...
        fog_copy_feather=2,
        fog_copy_vertical_jitter=0.20,
        fog_copy_min_visibility=0.05,
        data_vis=False,
        ...
        self.fog_copy_feather = int(fog_copy_feather)
        self.fog_copy_vertical_jitter = float(fog_copy_vertical_jitter)
        self.fog_copy_min_visibility = float(fog_copy_min_visibility)
    ...
    @staticmethod
    def _patch_visibility(patch: torch.Tensor) -> torch.Tensor:
        """Local visibility cue in [0, 1] from contrast and gradients."""
        if patch.numel() == 0:
            return patch.new_tensor(0.05)
        lum = 0.299 * patch[0:1] + 0.587 * patch[1:2] + 0.114 * patch[2:3]
        contrast = lum.std(unbiased=False)
        dx = (lum[..., :, 1:] - lum[..., :, :-1]).abs().mean() if lum.shape[-1] > 1 else lum.new_tensor(0.)
        dy = (lum[..., 1:, :] - lum[..., :-1, :]).abs().mean() if lum.shape[-2] > 1 else lum.new_tensor(0.)
        contrast_score = (contrast / 0.12).clamp(0.0, 1.0)
        gradient_score = (((dx + dy) * 0.5) / 0.20).clamp(0.0, 1.0)
        return (0.55 * contrast_score + 0.45 * gradient_score).clamp(0.02, 1.0)

    def _feather_mask(self, h: int, w: int, device, dtype) -> torch.Tensor:
        feather = max(0, min(self.fog_copy_feather, h // 2, w // 2))
        if feather == 0:
        ...
        dist_y = torch.minimum(yy + 1, h - yy)
        dist_x = torch.minimum(xx + 1, w - xx)
        alpha_y = (dist_y / (feather + 1)).clamp(0.0, 1.0)
        alpha_x = (dist_x / (feather + 1)).clamp(0.0, 1.0)
        return (alpha_y[:, None] * alpha_x[None, :]).unsqueeze(0)

    def _fog_consistent_patch(self, patch: torch.Tensor, target_patch: torch.Tensor) -> torch.Tensor:
        """Only add degradation so an already-foggy source patch matches the target region."""
        source_vis = self._patch_visibility(patch)
        target_vis = self._patch_visibility(target_patch).clamp_min(self.fog_copy_min_visibility)
        if source_vis <= target_vis:
            return patch
        transmission = (target_vis / source_vis.clamp_min(1e-4)).clamp(0.35, 1.0)
        airlight = target_patch.mean(dim=(-2, -1), keepdim=True).amax(dim=0, keepdim=True).clamp(0.60, 0.95)
        degraded = patch * transmission + airlight * (1.0 - transmission)
        if transmission < 0.75 and min(patch.shape[-2:]) >= 3:
            blurred = F.avg_pool2d(degraded.unsqueeze(0), 3, stride=1, padding=1).squeeze(0)
            blur_w = ((0.75 - transmission) / 0.40).clamp(0.0, 0.60)
            degraded = degraded * (1.0 - blur_w) + blurred * blur_w
        ...
        if random.random() < self.mixup_prob and self.mixup_epochs[0] <= self.epoch < self.mixup_epochs[-1]:
            ...
            for i in range(batch_size):
                ...
                for idx in selected_indices:
                    ...
                    if self.copyblend_type == 'fog_consistent' and new_h_px < img_height:
                        src_center_y = (y1_src + y2_src) / 2.0
                        jitter = self.fog_copy_vertical_jitter * img_height
                        low = max(new_h_px / 2.0, src_center_y - jitter)
                        high = min(img_height - new_h_px / 2.0, src_center_y + jitter)
                        center_y = random.uniform(low, max(low, high))
                        y1 = int(round(center_y - new_h_px / 2.0))
                        y1 = max(0, min(img_height - new_h_px, y1))
                    else:
                    ...
                    blend_areas.append(area.new_tensor(float(new_w_px * new_h_px)))
                    # mixup ratio
                    ...
                    target_patch = updated_images[i, :, y1:y2, x1:x2]
                    if self.copyblend_type == 'blend':
                        blended_patch = target_patch * beta + copy_patch_orig * (1 - beta)
                        updated_images[i, :, y1:y2, x1:x2] = blended_patch
                    elif self.copyblend_type == 'fog_consistent':
                        copy_patch = self._fog_consistent_patch(copy_patch_orig, target_patch)
                        alpha_mask = self._feather_mask(copy_patch.shape[-2], copy_patch.shape[-1],
                                                       copy_patch.device, copy_patch.dtype)
                        updated_images[i, :, y1:y2, x1:x2] = target_patch * (1.0 - alpha_mask) + copy_patch * alpha_mask
                    else:
