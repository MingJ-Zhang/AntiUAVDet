...
def main() -> None:
    ...
    from uavdetr_protocol import disable_online_and_extra_checks
    ...
    run_dir = project_root / 'runs' / f'uavdetr_{args.dataset}_768x1344_300e'
    output_dir = run_dir / 'evaluation' / 'test'
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / 'predictions_coco.json').write_text(
    ...
    ground_truth = {
        ...
        'categories': [{'id': 1, 'name': config['names'].get(0, config['names'].get('0', 'object'))}],
    }
    ...
    coco_dt = coco_gt.loadRes(str(output_dir / 'predictions_coco.json'))
