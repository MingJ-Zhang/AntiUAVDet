...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        if self.pil:  # use PIL
            ...
            try:
                ...
                size = font_size or max(round(sum(self.im.size) / 2 * 0.015), 12)
    ...
    def calculate_font_size(self, box_width, base_font_size=1, scale_factor=0.0005):
        font_size = base_font_size + (box_width * scale_factor)
        return font_size
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil:
            if rotated:
                ...
                self.draw.rectangle((*p1, *p2), width=self.lw, outline=color)  # box
            if label:
                w, h = self.font.getsize(label)
                outside = p1[1] >= h
                if p1[0] > self.im.size[0] - w:
                    p1 = self.im.size[0] - w, p1[1]
                color2, txt_color, alpha = (0, 0, 0), (255, 255, 255), 0.7
                ...
                # 先在 overlay 上绘制框
                if rotated:
                    draw_overlay.polygon([tuple(b) for b in box], width=self.lw, outline=color)
                else:
                    draw_overlay.rectangle((*p1, *p2), width=self.lw, outline=color)

                p2 = p1[0] + w, p1[1] - h if outside else p1[1] + h
                draw_overlay.rectangle(
                    (p1[0], p1[1] - h if outside else p1[1], p1[0] + w + 1, p1[1] + 1 if outside else p1[1] + h + 1),
                    fill=tuple(list(color2) + [int(alpha * 255)]),
                )
                draw_overlay.text((p1[0], p1[1] - h if outside else p1[1]), label, fill=txt_color, font=self.font)
            ...
            if label:
                box_width = abs(p2[0] - p1[0])
                self.sf = max(0.1, min(self.calculate_font_size(box_width), self.sf))
                ...
                p2 = p1[0] + w, p1[1] - h if outside else p1[1] + h
                color, txt_color, alpha = (0, 0, 0), (255, 255, 255), 0.7
                overlay = self.im.copy()
                cv2.rectangle(overlay, p1, p2, color, -1, cv2.LINE_AA)  # filled
                y1, y2 = min(p1[1], p2[1]), max(p1[1], p2[1])
                x1, x2 = min(p1[0], p2[0]), max(p1[0], p2[0])
                self.im[y1:y2, x1:x2] = cv2.addWeighted(
                    overlay[y1:y2, x1:x2], alpha,  # 叠加层部分
                    self.im[y1:y2, x1:x2], 1 - alpha,  # 原图部分
                    0
                )
                cv2.putText(
                    self.im,
