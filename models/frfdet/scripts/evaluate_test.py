...
def main() -> None:
    ...
    # This fork's list-source loader treats the whole Python list as one batch;
    # the predictor's ``batch`` argument does not split it again. Keep each
    # source list no larger than the requested inference batch to avoid OOM.
    source_chunk = min(args.source_chunk, args.batch)
    predictions = []
    for start in range(0, len(image_paths), source_chunk):
        source_paths = image_paths[start:start + source_chunk]
    ...
    if not predictions:
        raise RuntimeError(
            "model produced zero predictions; refusing to call COCO.loadRes on an empty list"
        )
