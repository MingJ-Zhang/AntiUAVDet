...
from .fog_modules import SpatialAoDERefiner
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 csp_type='csp',
                 fuse_op='cat',
                 use_spatial_aode=False,
                 aode_levels=2,
                 aode_iterations=2,
                 aode_memory=0.70,
                 aode_trust_floor=0.05,
                 aode_budget_min=0.02,
                 aode_budget_max=(0.18, 0.10),
                 aode_residual_scale=0.50,
                 return_evidence_state=False,
                 ):
        super().__init__()
        ...
        self.use_spatial_aode = bool(use_spatial_aode)
        self.return_evidence_state = bool(return_evidence_state)
        self.spatial_aode = SpatialAoDERefiner(
            channels=hidden_dim,
            num_levels=min(int(aode_levels), len(in_channels)),
            iterations=int(aode_iterations),
            memory=float(aode_memory),
            trust_floor=float(aode_trust_floor),
            budget_min=float(aode_budget_min),
            budget_max=tuple(aode_budget_max),
            residual_scale=float(aode_residual_scale),
        ) if self.use_spatial_aode else None
    ...
    def forward(self, feats):
        ...
        if self.spatial_aode is not None:
            outs, evidence_state = self.spatial_aode(outs)
            if self.return_evidence_state:
                return outs, evidence_state
